...
contract Raffle {
    ...
    uint256 public expiration; // Timeout in case That the Raffle was not carried out.
    address public RaffleOperator; // the crator of the Raffle
    uint256 public operatorTotalCommission = 0; // the total commission balance
    address public lastWinner; // the last winner of the Raffle
    uint256 public lastWinnerAmount; // the last winner amount of the Raffle
    ...
    // modifier to check if caller is the Raffle operator
    modifier isOperator() {
        require(
            (msg.sender == RaffleOperator),
            "Caller is not the Raffle operator"
        );
        _;
    }
    ...
    function RefundAll() public {
        require(block.timestamp >= expiration, "the Raffle not expired yet");
